...
import { CFMM_ADDRESS, CTEZ_FA12_ADDRESS } from '../utils/globals';
import { getTezosInstance } from '../../contracts/client';
import { getCTezFa12Contract, getLQTContract } from './fa12';
import { executeMethod, initContract } from '../../contracts/utils';
...
export const getLQTContractStorage = async (): Promise<any> => {
  const lqtContract = await getLQTContract(CTEZ_FA12_ADDRESS);
...
export const addLiquidity = async (args: AddLiquidityParams): Promise<WalletOperation> => {
  ...
  const CTezFa12 = await getCTezFa12Contract(CTEZ_FA12_ADDRESS);
...
export const removeLiquidity = async (
  ...
  const tezos = getTezosInstance();
  const LQTFa12 = await getLQTContract(CTEZ_FA12_ADDRESS);
  const batchOps: WalletParamsWithKind[] = await getTokenAllowanceOps(
    LQTFa12,
    userAddress,
    args.lqtBurned,
