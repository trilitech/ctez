...
class Fa12TransferTestCase(Fa12BaseTestCase):
    ...
    def test_should_fail_if_not_enough_balance(self) -> None:
        ...
        with self.raisesMichelsonError(Fa12.Errors.NOT_ENOUGH_BALANCE):
    ...
    def test_should_fail_if_sender_is_not_allowed(self) -> None:
        ...
        with self.raisesMichelsonError(Fa12.Errors.NOT_ENOUGH_ALLOWANCE):
    ...
    def test_should_transfer_correctly_if_sender_is_owner(self) -> None:
        ...
        prev_owner_balance = fa12.get_balance(owner)
        prev_recipient_balance = fa12.get_balance(recipient)
        transfer_amount = 10

        fa12.using(owner).transfer(owner, recipient, transfer_amount).send()
        self.bake_block()

        assert fa12.get_balance(owner) == prev_owner_balance - transfer_amount
        assert fa12.get_balance(recipient) == prev_recipient_balance + transfer_amount
    ...
    def test_should_transfer_correctly_if_sender_is_allowed(self) -> None:
        ...
        owner_prev_balance = fa12.get_balance(owner)
        recipient_prev_balance = fa12.get_balance(recipient)

        fa12.using(recipient).transfer(owner, recipient, transfer_amount).send()
        self.bake_block()

        assert fa12.get_balance(owner) == owner_prev_balance - transfer_amount
        assert fa12.get_balance(recipient) == recipient_prev_balance + transfer_amount
        assert fa12.get_allowance(owner, recipient) == allowed_amount - transfer_amount
