...
//   let transfer_self (from_ : address) (to_ : address) (value : nat) : operation = 
//     failwith "TODO"
...
//   let default_liqudity_owner = { liquidity_shares = 0n ; proceeds_owed = 0n ; subsidy_owed = 0n }
...
//   let deposit (t : t) ({ owner; amount_deposited; min_liquidity; deadline } : deposit) ()
//       : t * operation list
//     =
//     let d_liquidity =
...
//       }
//     in
//     let receive_self = transfer_self owner (Tezos.get_self_address ()) amount_deposited in
//     t, [ receive_self ]
...
//   let redeem (t : t) ({ to_; liquidity_redeemed; min_self_received; min_proceeds_received; min_subsidy_received; deadline } : redeem) : t * operation list = 
//     let () = assert_with_error (Tezos.get_now () <= deadline) "deadline has passed" in
//     let owner = Tezos.get_sender () in
//     let liquidity_owner = find_liquidity_owner t owner in
//     let () = assert_with_error (liquidity_owner.liquidity_shares >= liquidity_redeemed) "insufficient liquidity" in
//     let self_redeemed = redeem_amount liquidity_redeemed t.self_reserves t.total_liquidity_shares in
//     let owed_proceeds, proceeds_redeemed = 
//       (* Proceeds must account for netting the owed amount *)
//       let v = redeem_amount liquidity_redeemed t.proceeds_reserves t.total_liquidity_shares in
//       if v < liquidity_owner.proceeds_owed then
//         (abs (liquidity_owner.proceeds_owed - v), 0n)
//       else
//         (0n, abs (v - liquidity_owner.proceeds_owed))
//     in
//     let () = assert_with_error (proceeds_redeemed >= min_proceeds_received) "insufficient proceeds would be received" in
...
//         ; proceeds_owed = liquidity_owner.proceeds_owed + owed_proceeds
//         })
//     in
//     let t = { t with
...
//     (t, [receive_ctez; receive_subsidy; receive_tez])


//   type swap = 
//     { to_: address (** address that will own the 'self' tokens in the swap *)
//     ; deadline : timestamp (** deadline for the transaction *)
//     ; proceeds_amount : nat (** minimum amount of tez to buy *)    
//     ; min_self : nat (** the  *)
//     }
